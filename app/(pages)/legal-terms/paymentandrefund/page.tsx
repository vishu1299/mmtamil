...
import { getPaymentAndRefundData } from "./api/api";

interface PaymentAndRefundData {
  id: string;
  title: string;
  description: string;
  createdAt: string;
...
const PaymentAndRefund = () => {
  ...
  const [paymentAndRefundData, setPaymentAndRefundData] = useState<
    PaymentAndRefundData[]
  >([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await getPaymentAndRefundData();
        ...
        setPaymentAndRefundData(sortedData);
      } catch (error) {
        console.error("Error fetching payment and refund data:", error);
      }
    };
  ...
  if (paymentAndRefundData.length === 0) {
    return (
        ...
        <div className="col-span-5 mr-4 md:mr-8 lg:mr-16">
          <div className="flex-1 pl-4 pt-6">
            <p className="bg-black md:text-4xl text-xl md:font-bold font-semibold text-white py-10  flex items-center justify-center mt-22">
              {t("paymentRefundTitle")}
            </p>
            {paymentAndRefundData?.map((item, index) => (
              <div
                key={item.id}
                id={item.title?.toLowerCase().replace(/\s+/g, "-")}
                className={`pt-20 text-gray-800 w-full px-4 ${
                  index === paymentAndRefundData.length - 1 ? "mb-32" : ""
                }`}
              >
