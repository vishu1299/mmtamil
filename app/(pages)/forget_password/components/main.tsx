...
const ForgotPassword = () => {
  ...
  const formSchema = useMemo(
    () =>
      z
        .object({
          newPassword: z.string().min(8, t("zodPasswordMin")),
          confirmPassword: z.string(),
        })
        .refine((data) => data.newPassword === data.confirmPassword, {
          message: t("zodPasswordMismatch"),
  ...
  const sendForgotOtp = async (emailValue: string) => {
    try {
      ...
      toast.error(err.response?.data?.message || t("toastOtpSendFailed"));
    }
  };
  ...
  const handleVerificationOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result?.data?.code === 200 || result?.data?.code === 201) {
        ...
        toast.error(t("toastInvalidOtp"));
      }
    } catch (error: unknown) {
      const err = error as { response?: { data?: { message?: string } } };
      toast.error(err.response?.data?.message || t("toastInvalidOtp"));
    }
  };
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    if (!parsed.success) {
      ...
      const firstIssue =
        parsed.error.issues[0]?.message || t("toastPasswordMin");
    ...
    try {
      ...
      const err = error as { response?: { data?: { message?: string } } };
      const errorMessage =
        err.response?.data?.message || t("toastResetFailed");
  ...
  const handleSendOtp = (e: React.FormEvent) => {
    e.preventDefault();
    if (email) {
      void sendForgotOtp(email);
    }
  };

  const expiryDisplay = `${String(Math.floor(otpExpiry / 60)).padStart(2, "0")}:${String(otpExpiry % 60).padStart(2, "0")}`;
        ...
        <div className="w-full max-w-md">
          ...
          <form onSubmit={handleVerificationOtp}>
            ...
            <p className="text-[#6B6B6B] text-sm mt-4 text-left">
              {t("expiresIn", { time: expiryDisplay })}
            </p>
            ...
            <div className="mb-5">
              ...
              <div className="relative group">
                ...
                <button
                  ...
                  aria-label={
                    showPassword
                      ? t("toggleHidePassword")
                      : t("toggleShowPassword")
                  }
                >
                  {showPassword ? (
