...
import { getCookiePolicyData } from "./api/api";
...
const CookiePolicy = () => {
  const [cookiePolicyData, setcookiePolicyData] = useState<cookiePolicyData[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setcookiePolicyData(sortedData);
      } catch (error) {
            ...
            {cookiePolicyData?.map((item, index) => (
              <div
                ...
                className={`pt-20 text-gray-800 w-full px-4 ${
                  index === cookiePolicyData.length - 1 ? "mb-32" : ""
                }`}
              >
