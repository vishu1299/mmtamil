...
const PaymentSuccessPageContent = () => {
  ...
  // useEffect(() => {
  //   if (sessionId) {
  //     fetch(`http://localhost:5000/stripe/session-status?session_id=${sessionId}`)
  //       .then((res) => res.json())
  ...
  // }, [sessionId]);
    ...
    useEffect(() => {
      ...
      const fetchPaymentStatus = async () => {
        try {
          const response = await customAxios().get(`mmm/stripe/session-status?session_id=${sessionId}&package_id=${packageId}`);
          // const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}stripe/session-status?session_id=${sessionId}`, {
          //   method: "GET",
          //   headers: {
        ...
        <div className="flex flex-col items-center text-center">
          ...
          <div className="w-full space-y-4">
            <div className="flex justify-between flex-wrap">
              ...
              <p className="font-medium text-gray-800">{paymentData.session.payment_intent || "N/A"}</p>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-600">Amount Paid</span>
              <span className="font-medium text-gray-800">
                {paymentData.session.amount_total
                  ? `${(paymentData.session.amount_total / 100).toFixed(2)} ${paymentData.session.currency?.toUpperCase()}`
                  : "N/A"}
              </span>
            </div>
            <div className="flex justify-between">
              ...
              <span className="font-medium text-gray-800">
                {paymentData.session.payment_method_types?.[0]?.toUpperCase() || "N/A"}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-600">Date</span>
              <span className="font-medium text-gray-800">{formatDate(paymentData.session.created)}</span>
            </div>
          </div>
